...
class CLAS_PR(tf.keras.Model):
    def __init__(self, shape, num_classes, snapshots, wave_length, dx, distance, tipo_muestreo, kappa, mascara_lab, normalize_measurements, trainable_mask, float_dtype, complex_dtype, snr):
        ...
        self.muestreo_layer = Muestreo(snapshots = snapshots, wave_length = wave_length, 
                ...
                kappa = kappa, mascara_lab = mascara_lab, normalize_measurements = normalize_measurements, trainable=trainable_mask, 
...
class CLAS_PR_INIT(tf.keras.Model):
    def __init__(self, shape, num_classes, p, k_size, n_iterations, snapshots, wave_length, dx, distance, tipo_muestreo, kappa, mascara_lab, normalize_measurements, trainable_mask, float_dtype, complex_dtype, snr):
        super(CLAS_PR_INIT, self).__init__()
        self.p = p
        self.k_size = k_size
        self.kappa = tf.cast(kappa, float_dtype)
        ...
        self.Initialation = FSI_cell(p = p, k_size = k_size, kappa=self.kappa, tipo_muestreo = tipo_muestreo,float_dtype=float_dtype,complex_dtype=complex_dtype, name = "initialization_cell")
        self.classification_network = tf.keras.applications.MobileNetV2(input_shape=(*shape,3), classes=num_classes, weights=None, classifier_activation="softmax")
        self.conv_initial = tf.keras.layers.Conv2D(3, 3, padding="same", activation = None, name = "ConvInitial")
        self.ruido = customGaussian(snr = snr)
    ...
    def call(self, input):
        ...
        back_real = self.Unet_real(back_real)
        back_imag = self.Unet_imag(back_imag)

        x_real = self.conv_final_real(back_real)
        x_imag = self.conv_final_imag(back_imag)

        initialization_x = tf.concat([x_real, x_imag], axis=-1)
