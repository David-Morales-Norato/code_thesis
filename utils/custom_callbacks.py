...
class callback_class(tf.keras.callbacks.Callback):
  ...
  def __init__(self, results_folder, generator, ext='.png', num_saves = 100, num_epochs = None):
    ...
    self.n_epochs2skip = num_epochs//num_saves
  ...
  def on_epoch_end(self, epoch, logs=None):
    if epoch %self.n_epochs2skip == 0:
      print("CALLBACK EPOCH", epoch, "cada n epochs", self.n_epochs2skip)
      ...
      self.data_frame.iloc[epoch//self.n_epochs2skip,:] = [epoch, loss,val_loss,binary_accuracy,val_binary_accuracy,test_acc,precision_1,val_precision_1,test_precision,recall_1,val_recall_1,test_recall,f1_score,val_f1_score,test_f1_score]
      self.data_frame.to_csv(self.output_csv_file,index=False,header=True)
